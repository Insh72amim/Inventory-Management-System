...
if(isset($_POST['submit'])){
	...
	$sql_stmt="SELECT * FROM cashier WHERE username='$user'";
	...
	if($rows){
		$var1 = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_cashier.php";
		echo "<SCRIPT type='text/javascript'>
		alert('Sorry, the username entered already exists.');
		window.location.replace('$var1');
		</SCRIPT>";
	}
	else{
		$sql = "INSERT INTO cashier(first_name,last_name,staff_id,postal_address,phone,email,username,password,date) VALUES('$fname','$lname','$sid','$postal','$phone','$email','$user','$pas',NOW())";
		...
		if ($conn->query($sql) === TRUE) {
			...
			echo "<script type='text/javascript'>alert('main nhi bataunga');</script>";
...
<html>
<head>
	<title><?php echo $username;?> IITP MSMS </title>
	...
	<div id="content">
		...
		*/
        ...
        while($row = mysqli_fetch_array( $result )) {
        	...
        	?>
        	<td><a href="update_cashier.php?username=<?php echo $row['username']?>"><img src="images/update.png" width="35" height="35" border="0" /></a></td>
        	<td><a href="delete_cashier.php?cashier_id=<?php echo $row['cashier_id']?>"><img src="images/delete.png" width="35" height="35" border="0" /></a></td>
        	<?php
        } 
        // close table>
        echo "</table>";
