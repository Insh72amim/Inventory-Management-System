...
if(isset($_POST['submit'])){
	...
	$sql=mysqli_query($conn,"INSERT INTO stock(drug_name,category,description,company,supplier,quantity,cost,status,date_supplied) VALUES('$sname','$cat','$des','$com','$sup','$qua','$cost','$sta',NOW())");

	if ($conn->query($sql) === TRUE) {
		$var1 = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/stock_pharmacist.php";
		echo "<SCRIPT type='text/javascript'>
		alert('Inserted successfully into stock.');
		window.location.replace('$var1');
		</SCRIPT>";
	} else {
		$var2 = "Error: " . $sql . "<br>" . $conn->error;
		$var1 = "http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/stock_pharmacist.php";
		echo "<SCRIPT type='text/javascript'>
		alert('$var2');
		window.location.replace('$var1');
	...
	<div id="content">
		...
		*/
        ...
        while($row = mysqli_fetch_array( $result )) {
        	...
        	<td><a href="delete_stock.php?stock_id=<?php echo $row['stock_id']?>"><img src="images/delete.png" width="24" height="24" border="0" /></a></td>
        	<?php
        } 
        // close table>
        echo "</table>";
